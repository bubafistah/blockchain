...
used by Lethean. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Lethean's
...
> if interested in learning about Lethean p2p traffic!


## Header
This header is sent for every Lethean p2p message.
...
Lethean when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Lethean specific
...
### Requests
Requests are the basis of the admin protocol for Lethean. The `Q` bit must be
